...
if (isset($_POST['step']))
{
	if ($_POST['step']=="login")
	{
		...
		$db->query("select * from students where user='".$username."'");
		$res=$db->result();
		echo $res[0]['id'];
		if($res[0]['pass']==md5($password) )
		{
			...
			$_SESSION['admin.'.$company_name_en.'_student_id']=$res[0]['id'];
			$db->query("insert into students_login_log set
			student_id='".$_SESSION['admin.'.$company_name_en.'_student_id']."',
			date='$today',
			time='$today_time'
			");
		}
