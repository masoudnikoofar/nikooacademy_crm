...
if ($_POST[step]=="2")
{
	if ($invoice_id=="new")
	{
		$db->query("insert into invoices set buyer_id='$buyer_id',date='$date',time='$time',is_student=0");
		$db->query("select * from invoices where buyer_id='$buyer_id' order by id desc");
		$res=$db->result();
		$invoice_id=$res[0]['id'];
	}
		...
		<tr>
			...
			<td>
				<select name="invoice_id">
					<?php
					$db->query("select * from invoices where buyer_id='$buyer_id' and is_student=0 order by id desc");
					$res=$db->result();
					foreach ($res as $row)
					{
